...
OpenAI is text-only in nanograph today. Use Gemini when `@embed(...)` targets a `@media_uri(...)` field. See [embeddings.md](embeddings.md) for the current provider matrix and Gemini media limits.
