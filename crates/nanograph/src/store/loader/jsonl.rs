...
{"edge":"Knows","from":"Alice","to":"Bob"}"#;
        ...
        let err = load_jsonl_data(&mut no_seed, data).unwrap_err();
        assert!(
        ...
        load_jsonl_data_with_name_seed(&mut seeded, data, Some(&seed)).unwrap();
...
{"edge":"Knows","from":"Alice","to":"Bob"}"#;
        ...
        load_jsonl_data(&mut storage, data).unwrap();
