...
echo ""
DRY_RUN_OUT=$($NG migrate "$DB" --dry-run --format table 2>&1 || true)
echo "$DRY_RUN_OUT"
echo ""
...
echo ""
APPLY_OUT=$($NG migrate "$DB" --auto-approve --format table 2>&1 || true)
echo "$APPLY_OUT"
echo ""
...
cat > /tmp/sw_migrated.gq << 'QUERIES'
...
JEDI_POST=$($NG run --db "$DB" --query /tmp/sw_migrated.gq --name jedi --format csv)
JEDI_POST_COUNT=$(echo "$JEDI_POST" | tail -n +2 | wc -l | tr -d ' ')
...
HEROES_DESC=$($NG run --db "$DB" --query /tmp/sw_migrated.gq --name heroes_desc --format csv)
echo "$HEROES_DESC" | head -1 | grep -q "description" || fail "description column not found"
HEROES_DESC_COUNT=$(echo "$HEROES_DESC" | tail -n +2 | wc -l | tr -d ' ')
...
DUELS_POST=$($NG run --db "$DB" --query /tmp/sw_migrated.gq --name all_duels --format csv)
DUELS_POST_COUNT=$(echo "$DUELS_POST" | tail -n +2 | wc -l | tr -d ' ')
...
VETS_POST=$($NG run --db "$DB" --query /tmp/sw_migrated.gq --name battle_veterans --format csv)
VETS_POST_COUNT=$(echo "$VETS_POST" | tail -n +2 | wc -l | tr -d ' ')
...
WIELDERS_POST=$($NG run --db "$DB" --query /tmp/sw_migrated.gq --name wielders --format csv)
echo "$WIELDERS_POST" | head -1 | grep -q "weapon_type" || fail "weapon_type column missing"
WIELDERS_POST_COUNT=$(echo "$WIELDERS_POST" | tail -n +2 | wc -l | tr -d ' ')
...
HOMEWORLD_OUT=$($NG run --db "$DB" --query /tmp/sw_migrated.gq --name heroes_homeworld --format csv)
