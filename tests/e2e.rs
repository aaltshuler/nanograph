...
use hyperdb::catalog::build_catalog;
use hyperdb::ir::lower::lower_query;
use hyperdb::plan::planner::execute_query;
use hyperdb::query::parser::parse_query;
use hyperdb::query::typecheck::typecheck_query;
use hyperdb::schema::parser::parse_schema;
use hyperdb::store::graph::GraphStorage;
...
async fn run_query_test(query_str: &str, storage: Arc<GraphStorage>) -> Vec<RecordBatch> {
    ...
    execute_query(&ir, storage).await.unwrap()
}
...
"#,
    ...
    // With no param binding at runtime, this returns all people
    // because $name is unbound
